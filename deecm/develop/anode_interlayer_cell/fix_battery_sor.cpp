...
FixBatterySOR::FixBatterySOR(LAMMPS *lmp, int narg, char **arg) :
  ...
  T(303.0), // K
  F(96485.0), // C/mol
  sigma_el_SE(0.1), // Electrolyte conductivity for SE (S/m) (For LPSCl: https://www.mdpi.com/1996-1073/16/13/5100)
  sigma_el_CB(0.1),  // Electrolyte conductivity for CB (S/m) (temp value)
  sigma_ed_AM(6.29e7), // Electronic conductivity for AM (S/m) (For Ag: https://angstromsciences.com/elements-electrical-conductivity)
  sigma_ed_CB(6.1e4), // Electronic conductivity for CB (S/m) (For C: https://angstromsciences.com/elements-electrical-conductivity)
  sigma_ed_CC(5.96e7), // Electronic conductivity for CC (S/m) (For Cu: https://angstromsciences.com/elements-electrical-conductivity)
  sigma_ed_LI(1.08e7), // Electronic conductivity for Li (S/m) (For Li: https://angstromsciences.com/elements-electrical-conductivity)
  alpha_a(0.5), //unitless
  alpha_c(0.5),
  BC_bottom_type(4),      // Default: CC1 Anode
  BC_top_type(6),         // Default: CC2 Cathode
  ...
  current_flux_CC(0.05),  // Current flux at CC free end (A/m2)
  phi_el(NULL),
  phi_el_old(NULL),
  phi_ed(NULL),
  phi_ed_old(NULL),
  ...
  list(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"omega") == 0) {
      ...
      sigma_el_SE = force->numeric(FLERR,arg[iarg+1]);      // Electrolyte
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (type[i] == SE_type) {
          ...
          // Bottom boundary condition (CC, cathode)
...
//               double conductance = sigma_el_SE * contact_area / r_SI;
...
void FixBatterySOR::solve_sor_iteration()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // Skip top boundary (cathode) particles as they have fixed potentials el and ed
    ...
    // Determine electronic conductivity for particle i (4/5 have sigma_ed)
    double sigma_ed_i = 0.0;
    if (type[i] == AM_type) sigma_ed_i = sigma_ed_AM;
    else if (type[i] == CB_type) sigma_ed_i = sigma_ed_CB;
    else if (type[i] == BC_bottom_type) sigma_ed_i = sigma_ed_CC;
    else if (type[i] == BC_top_type) sigma_ed_i = sigma_ed_LI;

    // Determine ionic conductivity for particle i (SE and CB have sigma_el)
    double sigma_el_i = 0.0;
    if (type[i] == SE_type) sigma_el_i = sigma_el_SE;
    else if (type[i] == CB_type) sigma_el_i = sigma_el_CB;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (r < (radius[i] + radius[j])) {
        ...
        if (contact_area > 0.0) {
          ...
          else if (type[j] == BC_top_type) sigma_ed_j = sigma_ed_LI;

          // Determine ionic conductivity for particle j (SE and CB have sigma_el)
          double sigma_el_j = 0.0;
          if (type[j] == SE_type) sigma_el_j = sigma_el_SE;
          else if (type[j] == CB_type) sigma_el_j = sigma_el_CB;
          
          // === Electronic potential update === (CB, AM) Assume zero does not change
          // if (type[i] == CB_type || type[i] == AM_type) {
          ...
          // === Electrolyte potential update (SE and CB particles only) ===
          if (type[i] == SE_type || type[i] == CB_type) {
            if (type[j] == SE_type || type[j] == CB_type) {
              // ionic conduction
              double sigma_el_eff = 2.0 * sigma_el_i * sigma_el_j / (sigma_el_i + sigma_el_j); // Effective conductivity (harmonic mean)
              double conductance = sigma_el_eff * contact_area / r_SI;
              phi_el_sum += conductance * phi_el[j];
              coeff_el_sum += conductance;
