...
-------------------------------------------------------------------------
    ...
    Input script syntax:
      fix ID group-ID potential/cc keyword value ...

    Keywords:
      omega value               - CC relaxation parameter, 0 < omega < 2 (default 1.9)
      tolerance value           - convergence tolerance (default 1e-6)
      max_iter N                - maximum CC iterations per timestep (default 10000)
...
FixStyle(potential/cc,FixPotentialCC)

#else

#ifndef LMP_FIX_POTENTIAL_CC_H
#define LMP_FIX_POTENTIAL_CC_H

#include "fix.h"

namespace LAMMPS_NS {

class FixPotentialCC : public Fix {
 public:
  FixPotentialCC(class LAMMPS *, int, char **);
  virtual ~FixPotentialCC();
 ...
 protected:
  // CC solver parameters (user-configurable)
  double omega;              // CC relaxation parameter (0 < omega < 2)
...
E: Illegal fix potential/cc command

Self-explanatory. Check the input script syntax and compare to the
documentation for the command.

E: CC omega must be between 0 and 2
...
E: Fix potential/cc requires newton pair off
...
E: Invalid particle types for potential/cc

SE_type and Li_type must be valid atom types.

E: Invalid boundary condition types for potential/cc

Li_Ctype and Li_Atype must be valid atom types.

W: Battery CC did not converge

The CC solver did not converge within the maximum iterations.
