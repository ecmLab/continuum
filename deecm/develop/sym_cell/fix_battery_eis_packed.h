...
class FixBatteryEIS : public Fix {
 ...
 private:
  ...
  int    protocol;        // 0 = CC, 1 = CCD
  int    period;          // Half-cycle length in timesteps
  double increase_step;   // CCD current increment (A/m²)
  double cross_area;      // Cross-sectional area  (m²)
  int    debug_freq;      // Debug output interval  (timesteps)
