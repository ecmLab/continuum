...
% Scale the system equivalently according to the NMC particle size used in the current study
    sys.load    = 10*10.0*ncm.ave(par.iNcm)/ncm.ave(1);   % The loading is 10mg/cm^2 at 5um-NMC, other NMC are scaled accordingly
    sys.lx      = 30*ncm.ave(par.iNcm)/ncm.ave(1);        % The box size in x direction is 80um at 5um-NMC, other NMC are scaled accordingly
    sys.ly      = 30*ncm.ave(par.iNcm)/ncm.ave(1);        % The box size in y direction is 80um at 5um-NMC, other NMC are scaled accordingly   
    sys.lz      = 30*ncm.ave(par.iNcm)/ncm.ave(1);        % The initial thickness is 80um at 5um-NMC, other NMC are scaled accordingly
