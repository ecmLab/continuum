...
def path_finder(s):
    ...
    while not pq.empty():
        ...
        for v in adj[temp].keys():
            if v == parent[temp] or particle_type[v] == 'C':
                continue
...
data = {'num_Ag': num_Ag, 'num_active_Ag': num_active_Ag, 'thickness': SYS_thk, 'active_Ag_percent': vol_active_Ag/vol_Ag, 'path': shortest_paths}
