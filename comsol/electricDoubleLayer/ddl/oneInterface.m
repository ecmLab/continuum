% run_ddl.m
% Script to run the diffuse double layer COMSOL model and extract results
% Created: 2025-10-08
...
% Set parameters (matching ddl0.m exactly)
...
model.param.set('L_cell', 'xD*200', 'Cell length');
...
model.param.set('Cd_GCS', 'epsilon0_const/(xD/eps_PEO+xS/eps_S)', 'Capacitance per unit area (GCS theory, low potential limit)');
model.param.set('phiM', '10[mV]', 'Voltage of electrode vs PZC (DC)');
...
%% Add Results plots to COMSOL model for visualization
fprintf('\nAdding result plots to COMSOL model...\n');

% Point Graph 1: Current components vs time at left electrode
model.result.create('pg1', 'PlotGroup1D');
model.result('pg1').set('data', 'dset1');
model.result('pg1').label('Current Evolution at Bulk');

model.result('pg1').create('ptgr1', 'PointGraph');
model.result('pg1').feature('ptgr1').selection.set([2]);  % Right bulk (boundary 2)
model.result('pg1').feature('ptgr1').set('expr', 'i_cation');
model.result('pg1').feature('ptgr1').set('unit', 'A/m^2');
model.result('pg1').feature('ptgr1').set('descr', 'Cation current');
...
model.result('pg1').feature('ptgr2').selection.set([2]);
model.result('pg1').feature('ptgr2').set('expr', 'i_anion');
model.result('pg1').feature('ptgr2').set('unit', 'A/m^2');
model.result('pg1').feature('ptgr2').set('descr', 'Anion current');
model.result('pg1').feature('ptgr2').set('xdata', 'expr');
model.result('pg1').feature('ptgr2').set('xdataexpr', 't');
model.result('pg1').feature('ptgr2').set('xdataunit', 'ms');

model.result('pg1').create('ptgr3', 'PointGraph');
model.result('pg1').feature('ptgr3').selection.set([2]);
model.result('pg1').feature('ptgr3').set('expr', 'i_total');
model.result('pg1').feature('ptgr3').set('unit', 'A/m^2');
model.result('pg1').feature('ptgr3').set('descr', 'Total ionic current');
model.result('pg1').feature('ptgr3').set('xdata', 'expr');
model.result('pg1').feature('ptgr3').set('xdataexpr', 't');
model.result('pg1').feature('ptgr3').set('xdataunit', 'ms');

model.result('pg1').set('xlabel', 'Time (ms)');
model.result('pg1').set('ylabel', 'Current density (A/m^2)');
model.result('pg1').set('xlabelactive', true);
model.result('pg1').set('ylabelactive', true);

% Point Graph 2: Flux components at electrode
model.result.create('pg2', 'PlotGroup1D');
model.result('pg2').set('data', 'dset1');
model.result('pg2').label('Flux at Bulk');

model.result('pg2').create('ptgr1', 'PointGraph');
model.result('pg2').feature('ptgr1').selection.set([2]);  % Right bulk (boundary 2)
model.result('pg2').feature('ptgr1').set('expr', 'tds.tflux_cAx');
model.result('pg2').feature('ptgr1').set('unit', 'mol/(m^2*s)');
model.result('pg2').feature('ptgr1').set('descr', 'Cation total flux (x)');
model.result('pg2').feature('ptgr1').set('xdata', 'expr');
model.result('pg2').feature('ptgr1').set('xdataexpr', 't');
model.result('pg2').feature('ptgr1').set('xdataunit', 'ms');

model.result('pg2').create('ptgr2', 'PointGraph');
model.result('pg2').feature('ptgr2').selection.set([2]);
model.result('pg2').feature('ptgr2').set('expr', 'tds.tflux_cXx');
model.result('pg2').feature('ptgr2').set('unit', 'mol/(m^2*s)');
model.result('pg2').feature('ptgr2').set('descr', 'Anion total flux (x)');
model.result('pg2').feature('ptgr2').set('xdata', 'expr');
model.result('pg2').feature('ptgr2').set('xdataexpr', 't');
model.result('pg2').feature('ptgr2').set('xdataunit', 'ms');

model.result('pg2').set('xlabel', 'Time (ms)');
model.result('pg2').set('ylabel', 'Flux (mol/(m^2*s))');
model.result('pg2').set('xlabelactive', true);
model.result('pg2').set('ylabelactive', true);

% Point Graph 3: Diffusive vs Migration flux components
model.result.create('pg3', 'PlotGroup1D');
model.result('pg3').set('data', 'dset1');
model.result('pg3').label('Flux Components at Bulk');

model.result('pg3').create('ptgr1', 'PointGraph');
model.result('pg3').feature('ptgr1').selection.set([2]);
model.result('pg3').feature('ptgr1').set('expr', 'flux_cA_diffusive');
model.result('pg3').feature('ptgr1').set('unit', 'mol/(m^2*s)');
model.result('pg3').feature('ptgr1').set('descr', 'Cation diffusive flux');
model.result('pg3').feature('ptgr1').set('xdata', 'expr');
model.result('pg3').feature('ptgr1').set('xdataexpr', 't');
model.result('pg3').feature('ptgr1').set('xdataunit', 'ms');

model.result('pg3').create('ptgr2', 'PointGraph');
model.result('pg3').feature('ptgr2').selection.set([2]);
model.result('pg3').feature('ptgr2').set('expr', 'flux_cA_migration');
model.result('pg3').feature('ptgr2').set('unit', 'mol/(m^2*s)');
model.result('pg3').feature('ptgr2').set('descr', 'Cation migration flux');
model.result('pg3').feature('ptgr2').set('xdata', 'expr');
model.result('pg3').feature('ptgr2').set('xdataexpr', 't');
model.result('pg3').feature('ptgr2').set('xdataunit', 'ms');

model.result('pg3').create('ptgr3', 'PointGraph');
model.result('pg3').feature('ptgr3').selection.set([2]);
model.result('pg3').feature('ptgr3').set('expr', 'flux_cX_diffusive');
model.result('pg3').feature('ptgr3').set('unit', 'mol/(m^2*s)');
model.result('pg3').feature('ptgr3').set('descr', 'Anion diffusive flux');
model.result('pg3').feature('ptgr3').set('xdata', 'expr');
model.result('pg3').feature('ptgr3').set('xdataexpr', 't');
model.result('pg3').feature('ptgr3').set('xdataunit', 'ms');

model.result('pg3').create('ptgr4', 'PointGraph');
model.result('pg3').feature('ptgr4').selection.set([2]);
model.result('pg3').feature('ptgr4').set('expr', 'flux_cX_migration');
model.result('pg3').feature('ptgr4').set('unit', 'mol/(m^2*s)');
model.result('pg3').feature('ptgr4').set('descr', 'Anion migration flux');
model.result('pg3').feature('ptgr4').set('xdata', 'expr');
model.result('pg3').feature('ptgr4').set('xdataexpr', 't');
model.result('pg3').feature('ptgr4').set('xdataunit', 'ms');

model.result('pg3').set('xlabel', 'Time (ms)');
model.result('pg3').set('ylabel', 'Flux (mol/(m^2*s))');
model.result('pg3').set('xlabelactive', true);
model.result('pg3').set('ylabelactive', true);

% Point Graph 4: Current at left boundary for comparison
model.result.create('pg4', 'PlotGroup1D');
model.result('pg4').set('data', 'dset1');
model.result('pg4').label('Current at Left Electrode');

model.result('pg4').create('ptgr1', 'PointGraph');
model.result('pg4').feature('ptgr1').selection.set([1]);  % Left boundary
model.result('pg4').feature('ptgr1').set('expr', 'i_cation');
model.result('pg4').feature('ptgr1').set('unit', 'A/m^2');
model.result('pg4').feature('ptgr1').set('descr', 'Cation current (right)');
model.result('pg4').feature('ptgr1').set('xdata', 'expr');
model.result('pg4').feature('ptgr1').set('xdataexpr', 't');
model.result('pg4').feature('ptgr1').set('xdataunit', 'ms');

model.result('pg4').create('ptgr2', 'PointGraph');
model.result('pg4').feature('ptgr2').selection.set([1]);
model.result('pg4').feature('ptgr2').set('expr', 'i_anion');
model.result('pg4').feature('ptgr2').set('unit', 'A/m^2');
model.result('pg4').feature('ptgr2').set('descr', 'Anion current (right)');
model.result('pg4').feature('ptgr2').set('xdata', 'expr');
model.result('pg4').feature('ptgr2').set('xdataexpr', 't');
model.result('pg4').feature('ptgr2').set('xdataunit', 'ms');

model.result('pg4').create('ptgr3', 'PointGraph');
model.result('pg4').feature('ptgr3').selection.set([1]);
model.result('pg4').feature('ptgr3').set('expr', 'i_total');
model.result('pg4').feature('ptgr3').set('unit', 'A/m^2');
model.result('pg4').feature('ptgr3').set('descr', 'Total ionic current (right)');
model.result('pg4').feature('ptgr3').set('xdata', 'expr');
model.result('pg4').feature('ptgr3').set('xdataexpr', 't');
model.result('pg4').feature('ptgr3').set('xdataunit', 'ms');

model.result('pg4').set('xlabel', 'Time (ms)');
model.result('pg4').set('ylabel', 'Current density (A/m^2)');
model.result('pg4').set('xlabelactive', true);
model.result('pg4').set('ylabelactive', true);

fprintf('Result plots added to model\n');
