...
#include "NernstPlanckConvectionScaled.h"

registerADMooseObject("ecBetaApp", NernstPlanckConvectionScaled);

InputParameters
NernstPlanckConvectionScaled::validParams()
{
  ...
  params.addParam<Real>("scale",1,"the scaling factor");

  return params;
}

NernstPlanckConvectionScaled::NernstPlanckConvectionScaled(const InputParameters & parameters)
   : Kernel(parameters),
...
Real
NernstPlanckConvectionScaled::computeQpResidual()
{
  RealVectorValue ion_velocity = _F_RT * _diffusivity[_qp] * _grad_V[_qp];
  return -_test[_i][_qp]* _scale * _zIons * ion_velocity * _grad_u[_qp];
}

Real
NernstPlanckConvectionScaled::computeQpJacobian()
{
  RealVectorValue ion_velocity = _F_RT * _diffusivity[_qp] * _grad_V[_qp];
  return -_test[_i][_qp] * _scale * _zIons * ion_velocity * _grad_phi[_j][_qp];
}

Real
NernstPlanckConvectionScaled::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _V_var)
  {
   return  -_test[_i][_qp] * _scale * _zIons * _F_RT * _diffusivity[_qp] * _grad_phi[_j][_qp] * _grad_u[_qp];
  }
