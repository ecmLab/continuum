#include "ADChargedTransportAnisotropy.h"

registerADMooseObject("ecBetaApp", ADChargedTransportAnisotropy);

InputParameters
ADChargedTransportAnisotropy::validParams()
{
...
ADChargedTransportAnisotropy::ADChargedTransportAnisotropy(const InputParameters & parameters)
  : ADKernel(parameters),

// Get the parameters from the input file
    _diffusivity(parameters.get<MaterialPropertyName>("diffusivity")),
    _diffusivity_coef(getADMaterialProperty<RealTensorValue>(_diffusivity))
{
}

ADReal
ADChargedTransportAnisotropy::computeQpResidual()
{
