...
m0SE  = 5.0;    // mesh characteristic length for bottom points of SE
m1SE  = 0.025;   // mesh characteristic length for interface points of SE
m0Li  = 5.0;    // mesh characteristic length for top points of Li
m1Li  = 0.025;   // mesh characteristic length for bottom points of Li
m0Int = 0.025;   // mesh characteristic length for top points of interlayer
...
For i In {0 : nSE+1}
   ...
   np       = newp;
   // Point(np)= {x+dy*dInt/Sqrt(1+dy*dy), -dh/2.0*(1+Cos(Pi*x/dw))-dInt/Sqrt(1+dy*dy), 0, m1SE};
...
// //**** Creat reactionlayer geometry**/
// bRctp       = newp;        //Start from upper-left point of the reactionlayer
// Point(bRctp)= {0, -dh, 0, m1Int};
// Line(6)     = {eSEp, bRctp};
// For i In {1 : nInt+1}
//    x        = dw*i/(nInt + 1);
//    np       = newp;
//    Point(np)= {x, -dh/2.0*(1+Cos(Pi*x/dw)), 0, m1Int};
// EndFor
// eRctp       = newp;     //End point of the curved boundary of the reactionLayer
// Point(eRctp)= {lx, 0, m1Int};
// Line(7) = {bRctp:eRctp};
// Line(8) = {eRctp, bSEp};
// //*** Create loops from reactionlayer **/
// Curve Loop(9) = {6,7,8,3};

//**** Create Li geometry**/
Point(newp) = {lx, ly/4, 0, m0Li};
Point(newp) = {0,    ly/4, 0, m0Li};
bLip        = newp;     //Start point of the curved boundary of the Li
Point(bLip) = {0,  dInt-dh, 0, m1Int};
...
For i In {1 : nLi+1}
   ...
   Point(np)= {x, -dh/2.0*(1+Cos(Pi*x/dw))+dInt, 0, m1Int};
EndFor
eLip        = newp;     //End point of the curved boundary of the Li
Point(eLip) = {lx, dInt, 0, m1Int*8};
Line(13)    = {bLip: eLip};
Line(14)    = {eLip, bLip-2};
...
//*** Create blocks for the SE, reactionLayer, interLayer, and Li-metal **/
Plane Surface(1) = {5};
// Plane Surface(2) = {9};
Plane Surface(3) = {15};
Plane Surface(4) = {19};
...
//*** Mesh setting
//* Structure mesh on reaction layer
// Transfinite Surface {2} = {eSEp,bRctp,eRctp,bSEp};
// Transfinite Curve {7,3} = 171 Using Progression 1;
// Transfinite Curve {8,6} = 4 Using Progression 1;
