...
registerADMooseObject("ecmMechanicsApp", ADComputeEquilibriumPotential);
...
ADComputeEquilibriumPotential::ADComputeEquilibriumPotential(const InputParameters & parameters)
    : ADMaterial(parameters),
      ...
      _swelling_chemical_potential(_include_mechanical_effects ? &getADMaterialProperty<Real>(_base_name + "swelling_chemical_potential") : nullptr),
      _elastic_chemical_potential(_include_mechanical_effects ? &getADMaterialProperty<Real>(_base_name + "elastic_chemical_potential") : nullptr),       
      _equilibrium_potential(declareADProperty<Real>("equilibrium_potential")),
      _chemical_potential(declareADProperty<Real>("chemical_potential")),
      _state_of_charge(declareADProperty<Real>("state_of_charge")), 
      _diff_potential(getParam<MooseEnum>("potential").getEnum<DiffPotential>())
{
    ...
    if (!_has_conc && _reaction_rate_function)
    {
        mooseError("Need concentration for reaction rate function");
    }
}
    ...
    if (_has_conc && _include_conc)
    {
        ...
        if ((*_concentration)[_qp] > 0)
        {
            ...
            {   if ((*_concentration)[_qp] > 0 && (_c0 - (*_concentration)[_qp] > 0))
                    chem = std::log(_c0/(_c0 - (*_concentration)[_qp]));
//                    chem = std::log((*_concentration)[_qp]/(_c0 - (*_concentration)[_qp]));
//                    chem = std::log(cbar/(1 - cbar));
            }
