lx   = 1;       // width of the model, in unit um
...
EndFor
eSEp        = newp;   //End point of the curved boundary of the SE
Point(eSEp) = {-lx/2,0, 0, m1SE};
Line(3)  = {bSEp: eSEp};
Line(4)  = {eSEp, bSEp-2};
...
Point(bIntp)= {lx/4, 0, 0, m1Int};
Line(16)     = {eLip, bIntp};
eIntp       = newp;     //End point of the curved boundary of the interLayer
Point(eIntp)= {-lx/4, 0, 0, m1Int};
Line(17) = {bIntp:eIntp};
Line(18) = {eIntp, bLip};
...
Transfinite Surface {3}   = {bLip-2,bLip-1,bLip,eLip};
Transfinite Curve {11,13} = 301 Using Progression 1;
Transfinite Curve {12,14} = 151 Using Progression 1;
Transfinite Surface {4}   = {eIntp,bLip,eLip,bIntp};
Transfinite Curve {13,17} = 301 Using Progression 1;
Transfinite Curve {16,18} = 6 Using Progression 1;
Mesh.RecombineAll = 1;
