#include "MixSEInterfaceEn.h"

registerMooseObject("electro_chemo_mechApp", MixSEInterfaceEn);

InputParameters
MixSEInterfaceEn::validParams()
{
...
MixSEInterfaceEn::MixSEInterfaceEn(const InputParameters & parameters)
...
ADReal
MixSEInterfaceEn::computeQpResidual(Moose::DGResidualType type)
{
  ADReal r = 0;
...
/**
ADReal
MixSEInterfaceEn::computeQpJacobian(Moose::DGJacobianType type)
{
  ADReal jac = 0;
  ADReal k1 = std::exp(_reaction_rate[_qp] * _F_RT * (_potLi[_qp] + _u[_qp]));
  ADReal k2 = std::exp(- (1 - _reaction_rate[_qp]) * _F_RT * (_potLi[_qp] + _u[_qp]));
