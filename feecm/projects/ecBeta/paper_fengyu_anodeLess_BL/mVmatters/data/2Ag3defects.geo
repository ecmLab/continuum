...
For i In {0 : nSE+1}
   x        = -dw/2 + dw*i/(nSE + 1);
   Point(np)= {x-lx/6, dh*Cos(Pi*(x/dw)), 0, m1SE};
   Line(nl) = {np-1, np};
   np       = newp;
   nl       = newl;
EndFor
...
//+
Physical Curve("blockSE_btm")   = {bSEl:bAg1l,bD2l:bAg2l,bD3l:eSEl};
//+
Physical Curve("interface")   = {bAg1l+1:eAg1l,bAg2l+1:eAg2l};
//+
Physical Surface("blockSE") = {1};
//+
Physical Surface("blockAg") = {2,3};
