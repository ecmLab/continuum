lx   = 20;       // width of the model, in unit um
ly   = 20;       // height of the model, in unit um
...
np  = newp;
Point(np) = {coord[0], -ly/2, 0, 2};
Line(2) = {SEpList[nSE + 1], np};
coord = Point{SEpList[0]};
np = newp;
Point(np) = {lx/2, -ly/2, 0, 2};
Line(3) = {np-1, np};
np1 = newp;
Point(np1) = {lx/2,0, 0, 2};
Line(4) ={np, np1};
...
np = newp;
Point(np) = {lx/2, 0, 0, 0.1};
Line(9) = {np, intpListBot[0]};
np1 = newp;
coord = Point{intpListTop[0]};
Point(np1) = {lx/2, coord[1],0,0.1};
...
Point(np2) = { coord[0], ly/4, 0, 1}; 
np3 = newp; 
Point(np3) = {lx/2, ly/4, 0, 1};
Line(12) = {np1, np3};
Line(13) = {np3, np2};
