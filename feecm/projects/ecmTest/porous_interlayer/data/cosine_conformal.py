...
v_intbot_right = cubit.create_vertex(pore_width, v_intbot_curve_top.coordinates()[1], 0)
...
# right hand side of bottom of interlayer 
c_interlayer_bot = cubit.create_curve(v_intbot_curve_top, v_intbot_right)
...
v_inttop_right = cubit.create_vertex(pore_width, v_inttop_curve_top.coordinates()[1], 0)
...
# right hand side of top of interlayer 
c_interlayer_top = cubit.create_curve(v_inttop_curve_top, v_inttop_right)

c_interlayer_left = cubit.create_curve(v_inttop_curve_bot, v_intbot_curve_bot)
c_interlayer_right = cubit.create_curve(v_inttop_right, v_intbot_right)

# create loop for interlayer
c_interlayer = [c_interlayer_bot, c_interlayer_curve_bot, c_interlayer_left, c_interlayer_curve_top, c_interlayer_top, c_interlayer_right]
...
for i in range(0,nSE+1,1):
    ...
    if newx > 0 and newx <= pore_width:
        ...
        k = k + 1
# now add point at x = 0 since it is not guaranteed
v_metalbot_curve.append(cubit.create_vertex(0.0,v_intbot_curve_bot.coordinates()[1] + dInt,0))

v_metalbot_curve_top = v_metalbot_curve[0]
v_metalbot_curve_bot = v_metalbot_curve[-1]
v_metal_bot_right = cubit.create_vertex(pore_width, v_metalbot_curve_top.coordinates()[1], 0)
...
# right hand side of top of interlayer 
c_metal_bot = cubit.create_curve(v_metalbot_curve_top, v_metal_bot_right)


c_metal = []
v_metal_top_right = cubit.create_vertex(pore_width, ly/4, 0)
...
c_metal_right = cubit.create_curve(v_metal_bot_right, v_metal_top_right)
c_metal = [c_metal_bot, c_metal_curve_top, c_metal_left, c_metal_top, c_metal_right]
metal_surf= cubit.create_surface(c_metal)
surfaces = [se_surf, surf_interlayer, metal_surf]
...
for s in surfaces:
    str1 += str(s.id()) + ' '
    str2 += str(s.id()) + ' '
str1 += ' size auto factor 2'
cubit.cmd(str1)
cubit.cmd(str2)
# --- Refine edges surface by surface ---- 
str1 = " refine curve " + str(c_se_curve_top.id()) + ' ' + str(c_se_top.id())
str1 += ' numsplit 3 bias 1.5 depth 2 smooth'
cubit.cmd(str1)

# Now the interlayer 
str1 = " refine curve " + str(c_interlayer_curve_bot.id()) + ' ' + str(c_interlayer_bot.id())
str1 += ' ' + str(c_interlayer_curve_top.id()) + ' ' + str(c_interlayer_top.id())
str1 += ' numsplit 3 bias 1 depth 1 smooth'
cubit.cmd(str1)