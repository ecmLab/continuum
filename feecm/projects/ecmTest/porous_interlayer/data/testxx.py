...
interlayer_curves = c_interlayer_bot_list.copy()
# --- Recreate vertex and curve lists 
...
yprev = 0
for i in range(npart):
    v_inttop_curve.clear()
    # v_inttop_curve_ends.clear()
    if i > 0:
        vv = cubit.vertex(cubit.get_entities('vertex')[-1])
        print('Vertex {} at {} added to list'.format(vv.id(), vv.coordinates()))
        v_inttop_curve.append(vv)
        newxx = [ax for ax in xx if ax > xsplit[i] and ax <= xsplit[i+1]]
    else:
        newxx = [ax for ax in xx if ax >=xsplit[i] and ax <= xsplit[i+1]]
        # now add point at x = 0 since it is not guaranteed
        v_inttop_curve.append(cubit.create_vertex(0.0,v_intbot_curve_list[0][0].coordinates()[1] + dInt,0))
    ...
    v_inttop_curve_list.append(v_inttop_curve.copy())    

# Spline for bottom surface of interlayer
# loop through all vertex list and create curves 
c_interlayer_top_list = []
c_interlayer_curve_top = None
for v in v_inttop_curve_list:
    if len(v) > 2:
        str1 = "create curve spline location vertex " + str(v[0].id()) + " to " + str(v[-1].id())
        print(str1)
        cubit.cmd(str1)
        ...
        c_interlayer_curve_top = cubit.create_curve(v[0], v[-1])
    print('Creating line from {} at {} to {} at {}'.format(v[0].id(), v[0].coordinates(), v[-1].id(), v[-1].coordinates()))
    c_interlayer_top_list.append(c_interlayer_curve_top)
# --- Recreate vertex and curve lists 
# c_interlayer_bot_list.clear()
# v_intbot_curve_list.clear()
# v_intbot_curve.clear()
interlayer_curves = interlayer_curves + c_interlayer_top_list
# Now create surfaces and partition if necessary

if npart < 3:
    # for vb, vt in zip(v_intbot_curve_list, v_inttop_curve_list):
    vb = v_intbot_curve_list[0][0]
    vt = v_inttop_curve_list[0][0]
    c = cubit.create_curve(vb, vt)
    interlayer_curves.append(c)
    vb = v_intbot_curve_list[-1][-1]
    vt = v_inttop_curve_list[-1][-1]
    c = cubit.create_curve(vb, vt)
    interlayer_curves.append(c)
