...
do 
    ...
    do 
        ...
        echo $poro_state
        filebase="unsaturated_current_${c}_porosity_${p}_visco_1e4"
        fb2="'$filebase'"
        fb="Outputs/file_base=$fb2"
        run_cmd="mpirun -np 4 ~/github/electro_chemo_mech2/electro_chemo_mech2-opt -i test_unsaturated_real_units.i $ff $poro_state $dtmax $end_time $dtmax2 $fb"
        echo $run_cmd
        eval $run_cmd
    done
done
