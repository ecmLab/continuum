...
Geometry:
    ...
    R = 3.8818230 um                 (NMC quarter-circle radius)
...
-----------------------------------------------------------------
  ...
  stay distinct. block_NMC_right and block_LPSC_left land on the two
  independent arcs -> valid primary/secondary contact surfaces.

Sidesets / blocks produced (names match both inputs):
    block_NMC_right, block_LPSC_left            (CONTACT mode only; two arcs)
    block_bottom, block_right, block_top, block_left
    block_NMC, block_LPSC                       (element blocks)
...
OUT_FILE_CZM     = "mesh_czm_7030.msh"   # contact_CZM_3.i
OUT_FILE_CONTACT = "mesh_7030.msh"       # contact.i
...
def build_mesh(filename: str | None = None,
    ...
    #               surfaces (primary block_NMC_right / secondary block_LPSC_left).
    ...
    #               BreakMeshByBlockGenerator (block_NMC_block_LPSC etc.) anyway.
    if not conformal:
        gmsh.model.addPhysicalGroup(1, [arc_mid_nmc], PG["block_NMC_right"], "block_NMC_right")
        gmsh.model.addPhysicalGroup(1, [arc_mid_lps], PG["block_LPSC_left"],  "block_LPSC_left")
    ...
    gmsh.model.addPhysicalGroup(2, [s_lps], PG["block_LPSC"], "block_LPSC")
