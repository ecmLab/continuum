...
\section{Temperature-dependent design window}
\label{sec:T-window}

The preceding chapters assumed an isothermal cell at the cure
temperature.  The DARPA Promethean program requires the cell to
operate over an ambient temperature window of $-20$ to $+60$~$^\circ$C,
so the same design-window analysis must be repeated at each end of
that envelope.  Differential thermal expansion across the
heterogeneous stack---metal collectors, polymer-bonded electrodes,
gel-polymer separator, polymer + ceramic package---shifts the
through-thickness contact pressure that the cure-shrinkage wrap
establishes, and therefore shifts the $(\varepsilon_{\rm GPE},
\varepsilon_{\rm pkg})$ feasibility window.

\subsection{Thermal eigenstrain}

We append a third eigen factor to the multiplicative split of
Eq.~\eqref{eq:multi-split},
\begin{equation}
\mathbf{F}_g = \mathbf{F}_c\, \mathbf{F}_l\, \mathbf{F}_t , \qquad
\mathbf{F}_t = \sqrt[3]{1 + \alpha_T\,(T - T_{\rm ref})}\;\mathbf{I},
\label{eq:Ft}
\end{equation}
with $\alpha_T$ the block-dependent volumetric coefficient of thermal
expansion ($\alpha_T = 3\alpha_T^{\rm lin}$ for isotropic
homogenization).  Cure is treated as isothermal at $T_{\rm ref} =
25$~$^\circ$C, so $\mathbf{F}_t \equiv \mathbf{I}$ during the cure
ramp $t \in [0,1]$ and all earlier chapters remain unchanged.  After
cure, the cell temperature ramps from $T_{\rm ref}$ to a prescribed
operating temperature $T_{\rm op}$ and is then held; the static
post-equilibration $\Pi_c$ is reported below.  Repeating the analysis
at $T_{\rm op} \in \{-20, +25, +60\}$~$^\circ$C samples the cold,
room-T, and hot operating points of the DARPA envelope.

\begin{table}[H]
\centering
\caption{Per-block coefficient of thermal expansion.  Linear values
are representative literature midpoints for homogenized composite
layers; the volumetric value $\alpha_T^{\rm vol} = 3\alpha_T^{\rm
lin}$ is what enters Eq.~\eqref{eq:Ft}.  These should be replaced
with values measured on the actual stack materials before
quantitative go/no-go decisions are taken.}
\label{tab:CTE}
\begin{tabular}{lcc}
\toprule
Block  & $\alpha_T$ [1/K] \\
\midrule
Cu current collector       & $5.1\times 10^{-5}$ \\
Graphite anode (composite)  & $1.5\times 10^{-5}$ \\
GPE / separator            & $4.5\times 10^{-4}$ \\
LVPF cathode (composite)   & $3.9\times 10^{-5}$ \\
Al current collector        & $6.9\times 10^{-5}$ \\
Package layer              & $1.8\times 10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}

The GPE separator dominates the CTE budget at $\alpha_T^{\rm lin} =
150$~ppm/K, an order of magnitude larger than the metal collectors;
the package layer is intermediate.  Differential expansion between
GPE, package, and the active electrodes is the principal driver of
the temperature-dependent modulation reported below.

\subsection{Temperature-dependent design map}

We repeat the four-axis design map of Section~\ref{sec:design} with
the third axis ($\sigma_{\rm stack}$) replaced by the operating
temperature $T_{\rm op}$, holding $\sigma_{\rm stack} = 0$ (the
pressure-free condition) and $E_{\rm sep} = 10$~MPa throughout.
Figure~\ref{fig:T-heatmap} shows the $\Pi_c$ heatmap over the
$(\varepsilon_{\rm GPE},\,\varepsilon_{\rm pkg})$ grid at the three
DARPA operating points.  The qualitative structure is unchanged from
Fig.~\ref{fig:heatmap}: $\Pi_c$ rises with $\varepsilon_{\rm pkg}$,
falls gently with $\varepsilon_{\rm GPE}$, and the white $\Pi_c = 1$
contour bounds the feasible window.  The feasibility window shifts
\emph{systematically} with $T_{\rm op}$.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/T_design_map_heatmap.pdf}
\caption{Contact-stability metric $\Pi_c$ over the
$(\varepsilon_{\rm GPE},\,\varepsilon_{\rm pkg})$ grid at the three
DARPA operating temperatures (slice at $\sigma_{\rm stack}=0$,
$E_{\rm sep}=10$~MPa).  Cooling to $-20^\circ$C contracts the
package envelope, tightens the wrap, and shifts the white
$\Pi_c=1$ contour toward lower package shrink.  Heating to
$+60^\circ$C expands the envelope, slackens the wrap, and shifts
the contour toward higher package shrink.  The hot corner with
high GPE shrink and low package shrink crosses into $\Pi_c < 0$
(stack-averaged tension), confirming that low-package, high-GPE
combinations are the most temperature-sensitive.}
\label{fig:T-heatmap}
\end{figure}

\subsection{Package shrink remains the dominant lever}

Figure~\ref{fig:T-lines-pkg} reports $\Pi_c$ vs.\ $\varepsilon_{\rm
pkg}$ at fixed $\varepsilon_{\rm GPE}$ for each $T_{\rm op}$.  At
all three temperatures the slope is essentially constant ($\sim$17
per unit $\varepsilon_{\rm pkg}$, the same as the room-T result in
Fig.~\ref{fig:lines-pkg}), and the $T_{\rm op}$ families are
nearly parallel vertical translates of each other.  Cooling to
$-20^\circ$C shifts each family upward by $\Delta\Pi_c\approx
+0.11$ and lowers the $\Pi_c = 1$ crossing from $\varepsilon_{\rm
pkg}\approx 0.07$ to $\varepsilon_{\rm pkg}\approx 0.06$ at the
nominal $\varepsilon_{\rm GPE} = 0.05$; heating to $+60^\circ$C
shifts each family downward by $\Delta\Pi_c\approx -0.09$ and
raises the crossing to $\varepsilon_{\rm pkg}\approx 0.08$.  In
other words, the package shrink required to clear target at the
hot end is $\sim$$1$ percentage point higher than at the cold end.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/T_Pi_c_vs_eps_pkg.pdf}
\caption{$\Pi_c$ vs.\ $\varepsilon_{\rm pkg}$ at four
$\varepsilon_{\rm GPE}$ levels, three operating temperatures
($\sigma_{\rm stack}=0$, $E_{\rm sep}=10$~MPa).  The $T_{\rm op}$
panels are near-parallel vertical translates: the cold panel sits
$\sim$0.11 above the room-T panel, and the hot panel sits
$\sim$0.09 below.  Dashed line marks the $\Pi_c=1$ target.}
\label{fig:T-lines-pkg}
\end{figure}

\subsection{GPE-shrink trend persists across temperature}

Figure~\ref{fig:T-lines-GPE} shows the dual lever: $\Pi_c$ vs.\
$\varepsilon_{\rm GPE}$ at fixed $\varepsilon_{\rm pkg}$, panels by
$T_{\rm op}$.  The mild negative slope identified at room
temperature (Fig.~\ref{fig:lines-GPE}, separator unloading by GPE
shrink) persists at both extremes with similar magnitude.  No
qualitatively new failure mode appears in either the cold or hot
panel: the same low-package, high-GPE corner that is marginal at
room temperature becomes the first to enter tension at $+60^\circ$C
($\Pi_c = -0.05$ at $\varepsilon_{\rm GPE} = 0.12,\,\varepsilon_{\rm
pkg} = 0.02$).  The design implication of
Section~\ref{sec:design}---that $\varepsilon_{\rm GPE}$ must be
co-designed with $\varepsilon_{\rm pkg}$, not independently
maximized---therefore carries over unchanged to the DARPA
operating window.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/T_Pi_c_vs_eps_GPE.pdf}
\caption{$\Pi_c$ vs.\ $\varepsilon_{\rm GPE}$ at four
$\varepsilon_{\rm pkg}$ levels, three operating temperatures
($\sigma_{\rm stack}=0$, $E_{\rm sep}=10$~MPa).  The mild negative
slope (separator unloading by GPE shrink) is invariant in
magnitude; only the vertical baseline shifts with $T_{\rm op}$.
Dashed line marks the $\Pi_c=1$ target.}
\label{fig:T-lines-GPE}
\end{figure}

\subsection{Net design-window shift across the DARPA envelope}

Combining the three panels, the operating window at $T_{\rm op} =
+60^\circ$C is a shrunken subset of the room-T window: the contour
that defines $\Pi_c \ge 1$ retreats from $\varepsilon_{\rm pkg}
\approx 0.07$ at $25^\circ$C to $\approx 0.08$ at $60^\circ$C
(holding $\varepsilon_{\rm GPE} = 0.05$, $\sigma_{\rm stack} = 0$).
At $T_{\rm op} = -20^\circ$C the window expands, with the contour
retreating to $\varepsilon_{\rm pkg} \approx 0.06$.  The net
$\Delta\Pi_c$ across the full DARPA envelope is $\approx 0.20$ at
any fixed design point.  A practical design rule is to size
$\varepsilon_{\rm pkg}$ for the hot endpoint (the worst case in the
envelope) and accept extra margin at colder operation.

The metal/plastic interface inside a metallized current collector
(a follow-on geometric option for weight optimization) inherits the
same operating-T sensitivity: its in-stack location places it under
the same through-thickness compression that closes the
electrode$|$separator interfaces already resolved here, and the
CTE mismatch across it ($\Delta\alpha_T^{\rm lin} \approx
(\alpha_{\rm pkg} - \alpha_{\rm Cu}) = 43$~ppm/K) is bounded
below the largest mismatch in the stack (GPE-vs-electrode,
$\sim$130--145~ppm/K).  Because that latter, more severe mismatch
remains in compression throughout the DARPA envelope at any
design point that clears target at $+60^\circ$C, an embedded
metal/plastic current-collector interface inherits at least the
same compressive margin.  Explicit resolution would require
splitting the current-collector block into a metal/polymer/metal
sandwich and recomputing $\Pi_c$ on the new sideset; this is
straightforward in the existing model and is recommended as a
follow-on once the candidate current-collector chemistry and
...
parameters.  Across the DARPA operating envelope
(Section~\ref{sec:T-window}), the design map shifts by
$\Delta\Pi_c\approx +0.11$ at $-20^\circ$C and $\approx -0.09$ at
$+60^\circ$C relative to room temperature: the package shrink
required to clear target at the hot endpoint is $\sim$1 percentage
point higher than at the cold endpoint.  Sizing $\varepsilon_{\rm
pkg}$ for the hot endpoint is the practical design rule, and the
same conclusion extends by inheritance to any metallized
current-collector interface embedded in the stack.
