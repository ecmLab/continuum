...
EEL = (HERE / ".." / ".." / ".." / "eel" / "eel-opt").resolve()
INPUT = HERE / "pressure_free_stack.i"
OUT_DIR = HERE / "sweeps"

# Temperature-grid sweep: at fixed sigma_stack = 0 and E_sep = 10 MPa,
# repeat the (eps_GPE x eps_pkg) design map at three operating
# temperatures spanning the DARPA Promethean envelope.  Cure runs at
# T_REF; T then ramps to T_op over t in [1, 1+T_RAMP].
T_REF = 25.0
T_OP_GRID = [-20.0, 25.0, 60.0]
T_RAMP = 0.5
T_GRID_END_TIME = 1.6                 # past the ramp; steady at T_op
T_GRID_SIGMA_STACK = 0.0
T_GRID_E_SEP = 10.0
...
def run_T_grid_point(run_idx: int, eps_GPE: float, eps_pkg: float,
                     T_op: float) -> dict:
    """One (eps_GPE, eps_pkg, T_op) design point at sigma_stack = 0.
    Returns the final-time postprocessor row."""
    file_base = f"sweeps/T_grid_{run_idx:03d}"
    cmd = [
        str(EEL),
        "-i", str(INPUT),
        f"eps_GPE={eps_GPE}",
        f"eps_pkg={eps_pkg}",
        f"sigma_stack={T_GRID_SIGMA_STACK}",
        f"E_sep={T_GRID_E_SEP}",
        f"T_op={T_op}",
        "cycle_amplitude=0.0",
        f"end_time={T_GRID_END_TIME}",
        f"Outputs/file_base={file_base}",
        "Outputs/exodus=false",
    ]
    print(f"[Tg{run_idx:03d}] T_op={T_op:+5.1f}  "
          f"eps_GPE={eps_GPE:.3f}  eps_pkg={eps_pkg:.3f}", flush=True)
    proc = subprocess.run(cmd, cwd=HERE, capture_output=True, text=True)
    if proc.returncode != 0:
        sys.stderr.write(proc.stdout[-2000:] + "\n")
        sys.stderr.write(proc.stderr[-2000:] + "\n")
        raise RuntimeError(f"T-grid run {run_idx} failed (returncode {proc.returncode})")
...
def sweep_T_grid() -> None:
    """Repeat the (eps_GPE x eps_pkg) design map at each T_op in
    T_OP_GRID.  Writes sweeps/T_grid.csv (one row per design point)."""
    print(f"Running temperature-grid sweep at sigma_stack={T_GRID_SIGMA_STACK}, "
          f"E_sep={T_GRID_E_SEP} MPa, T_op in {T_OP_GRID} C.")
    records = []
    run_idx = 0
    for T_op in T_OP_GRID:
        for eps_GPE in EPS_GPE_GRID:
            for eps_pkg in EPS_PKG_GRID:
                row = run_T_grid_point(run_idx, eps_GPE, eps_pkg, T_op)
                ...
                rec = {
                    ...
                    "eps_GPE": eps_GPE,
                    "eps_pkg": eps_pkg,
                    "sigma_stack_MPa": T_GRID_SIGMA_STACK,
                    "E_sep_MPa": T_GRID_E_SEP,
                ...
                records.append(rec)
                print(f"      pc_avg={rec['pc_avg_MPa']:.3f} MPa  "
                      f"pc_worst={rec['pc_worst_MPa']:+.3f} MPa  "
                      f"Pi_c={rec['Pi_c']:.3f}")
                run_idx += 1

    out_csv = OUT_DIR / "T_grid.csv"
...
def main() -> int:
    ...
    # Skip the full 192-point design map if invoked with "T" as argv[1];
    # run only the temperature side-sweep.  No-arg call runs both.
    ...
    # Also run the temperature-grid sweep for the new chapter.
    sweep_T_grid()
    return 0
