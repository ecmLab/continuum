...
sigma_e = 1e-3    # S/m (ionic conductivity in 1wt%LiClO4-PEO electrolyte)
...
D_ClO4 = 2.4e-12  # m^2/s (ClO4- diffusivity in electrolyte, assumed similar to Li+)
t_plus = 0.5      # Li+ transference number in PEO-LiClO4 electrolyte
...
[Materials]
  ...
  [li_mobility]
    ...
    # Ambipolar mobility: accounts for both Li+ and ClO4- migration
    # D_ambipolar ≈ 2*D_Li*D_ClO4/(D_Li + D_ClO4) = D_Li (when D_Li ≈ D_ClO4)
    expression = 'D_Li*max(c,1e-6)/${R}/T'  # Ambipolar approximation
  []
