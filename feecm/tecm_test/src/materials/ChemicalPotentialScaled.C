...
#include "ChemicalPotentialScaled.h"

registerMooseObject("tecm_testApp", ChemicalPotentialScaled);

InputParameters
ChemicalPotentialScaled::validParams()
{
...
ChemicalPotentialScaled::ChemicalPotentialScaled(const InputParameters & parameters)
...
void
ChemicalPotentialScaled::computeProperties()
{
  ...
  for (_qp = 0; _qp < _qrule->n_points(); _qp++)
  {
    ...
    {
      _mu[_qp] += _R*_T*_test[i][_qp] * sol(i);
      _grad_mu[_qp] +=_R*_T*_L0[_qp]*_grad_test[i][_qp] * sol(i);
    }
  }
}

TecmUtils::ADRealEigenVector
ChemicalPotentialScaled::L2Projection()
{
