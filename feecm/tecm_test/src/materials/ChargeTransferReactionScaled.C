...
#include "ChargeTransferReactionScaled.h"

registerMooseObject("tecm_testApp", ChargeTransferReactionScaled);

InputParameters
ChargeTransferReactionScaled::validParams()
{
...
ChargeTransferReactionScaled::ChargeTransferReactionScaled(const InputParameters & parameters)
...
void
ChargeTransferReactionScaled::initQpStatefulProperties()
{
  _i[_qp] = 0;
}

void
ChargeTransferReactionScaled::computeQpProperties()
{
  ...
  _i[_qp] =  (std::exp(coef * eta) - std::exp(-coef * eta));
