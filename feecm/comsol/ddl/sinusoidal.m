% sinusoidal.m
% Two-interface diffuse double layer model - Sinusoidal Voltage Excitation
% Based on twoInterface_ground with sinusoidal voltage at fixed frequency
% Left electrode: V_dc + V_ac*sin(2*pi*f*t), Right electrode: grounded with Stern
% Created: 2025-10-09
...
fprintf('Building TWO-INTERFACE SINUSOIDAL model...\n');
fprintf('Configuration: Left at V_ac*sin(wt) + grounded (both with Stern)\n\n');
...
% Right electrode variables (grounded metal, but has Stern layer!)
...
fprintf('\nTwo-interface sinusoidal model built successfully!\n');
fprintf('  Model tag: %s\n', char(model.tag()));
fprintf('  Configuration: Asymmetric - V_ac*sin(wt) + grounded (both with Stern)\n');
fprintf('  Left electrode: %.1f mV * sin(2*pi*%.0f*t) (with Stern layer)\n', V_ac*1000, freq);
