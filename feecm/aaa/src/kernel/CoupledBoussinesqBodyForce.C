...
#include "Function.h"
// A mask function is utilized to input 0 body force at bubble and 1 body force at liquid
//https://github.com/idaholab/moose/blob/devel/modules/phase_field/test/tests/MaskedBodyForce/MaskedBodyForce_test.i


template <>
InputParameters
validParams<CoupledBoussinesqBodyForce>()
{
...
CoupledBoussinesqBodyForce::CoupledBoussinesqBodyForce(const InputParameters & parameters)
  : Kernel(parameters), 
...
Real
CoupledBoussinesqBodyForce::computeQpResidual()
{
  // -rho * g * phi
  //RealVectorValue gravity_term = _gravity;
  //return -gravity_term*_rho_bq[_qp] * _test[_i][_qp];
  return -_rho_bq[_qp] *  _mask[_qp] * _gravity(_component) * _test[_i][_qp];
}

Real
CoupledBoussinesqBodyForce::computeQpJacobian()
{
  return 0.0;
}

Real
CoupledBoussinesqBodyForce::computeQpOffDiagJacobian(unsigned int jvar)
{
  //if (jvar == _rho_bq_var_number)
  //  return -_phi[_j][_qp] * _acceleration * _test[_i][_qp];
// the density in CoupledBoussinesqBodyForce kernel is brought
