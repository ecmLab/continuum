...
BackstressDiffusion::BackstressDiffusion(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
BackstressDiffusion::computeQpResidual()
{
  ...
  return stress_velocity * _u[_qp] * _grad_test[_i][_qp];
}

Real
BackstressDiffusion::computeQpJacobian()
{
  RealVectorValue stress_velocity =
    _D[_qp]* _omega * (1.0/(_kb*_T_c)) * _grad_stress[_qp];

  return stress_velocity * _phi[_j][_qp] * _grad_test[_i][_qp];
}

Real
BackstressDiffusion::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _stress_var)
  {
    _D[_qp]* _omega * (1.0/(_kb*_T_c)) * _u[_qp] *
