...
HeatCapacityMaterial::HeatCapacityMaterial(const InputParameters & parameters) :
    Material(parameters),
    ...
    _cprhokmu(declareProperty<Real>("specific_heat")),
    ...
    //write a unique name for convection velocity as there are multiple material objects in a same block that depend on same independent variable T
...
void
HeatCapacityMaterial::computeQpProperties()
{
