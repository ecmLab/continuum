...
template<>
InputParameters validParams<DensityMaterial>()
{
...
DensityMaterial::DensityMaterial(const InputParameters & parameters) :
    Material(parameters),
    // Declare that this material is going to provide a Real
    // valued property named "diffusivity" that Kernels can use.
    _cprhokmu(declareProperty<Real>("density")),
    ...
    //As there are multiple material objects in a same block dependent on same indenpendent variable T,
    // give a unique name to the declared property
     _convection_velocity(declareProperty<RealGradient>("dens_convection_velocity")),
...
void
DensityMaterial::computeQpProperties()
{
