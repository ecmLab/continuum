...
template<>
InputParameters validParams<ThermalConductivityMaterial>()
{
...
ThermalConductivityMaterial::ThermalConductivityMaterial(const InputParameters & parameters) :
    Material(parameters),
    // Declare that this material is going to provide a Real
    // valued property named "diffusivity" that Kernels can use.
    //_cprhokmu(declareProperty<Real>("material property")), 
     _cprhokmu(declareProperty<Real>("thermal_conductivity")),
    ...
    // give a unique name to the declared property
     _convection_velocity(declareProperty<RealGradient>("thermal_convection_velocity")),
...
void
ThermalConductivityMaterial::computeQpProperties()
{
