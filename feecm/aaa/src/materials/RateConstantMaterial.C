...
template<>
InputParameters validParams<RateConstantMaterial>()
{
...
RateConstantMaterial::RateConstantMaterial(const InputParameters & parameters) :
    Material(parameters),
    ...
    _cprhokmu(declareProperty<Real>("k_prcpt")), //k_prcpt is the name associated with PrecipitationBCS kernel
    // the unit of k_prcpt is m/s
    ...
    // give a unique name to the declared property
     _convection_velocity(declareProperty<RealGradient>("kprecpt_convection_velocity")),
...
void
RateConstantMaterial::computeQpProperties()
{
