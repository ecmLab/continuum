...
ScaledPolynomialFreeEnergy::ScaledPolynomialFreeEnergy(const InputParameters & parameters) :
    ...
    _order(getParam<MooseEnum>("polynomial_order"))
{
  ...
  {
    case 0: //4th order
      free_energy(_c, _W, _a) = 1.0e-15*pow(2.0, 4.0)*_W*pow(_c - _a, 2)*pow(1 - _c - _a, 2);
      break;
    case 1: //6th order
      free_energy(_c, _W, _a) = 1.0e-15*pow(2.0, 6.0)*_W*( 2.0*pow(_c, 6) -
                                6.0*pow(_c, 5) +
      ...
      break;
    case 2: //8th order
      free_energy(_c, _W, _a) = 1.0e-15*pow(2.0, 8.0)*_W*(3.0*pow(_c, 8) -
                                12.0*pow(_c,7) +
      ...
      break;
    default:
      mooseError("Error in ScaledPolynomialFreeEnergy: incorrect polynomial order");
  }
...
//ScaledPolynomialFreeEnergy::ScaledPolynomialFreeEnergy(const std::string & deprecated_name, InputParameters parameters) :
