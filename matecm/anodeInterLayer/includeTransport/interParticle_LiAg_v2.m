...
for ip = 1 : Np
    ...
    M12(ip,ip:ip+1) = [1,-1];
    M43(ip,ip)      = La(ip);
    Mtm(ip,ip:ip+1) = [1,-1]/dia(ip)^2;
end
M42 = [eye(Np),zeros(Np,1)];     % In unit 1
M41 = M43 + (40/pi*Hx^2)*sgm_Li*R_Pt*Mtm;  % In unit um
...
DD  = zeros(Np,Np+1);
cnt = -24*10^5*Hx^2*sgm_Li/(pi*rho*FF^2);
for ip = 1 : Np
    DD(ip,ip:ip+1) = cnt*[1,-1]/dia(ip)^3;       % In unit mol/J * um/s
end
...
for it = 1:length(tmStp)
    ...
    figure(ifg+1)
    plot(xxp-Lx,it_ionic,'-r', xxp-Lx, it_elec,'-b')
    xlabel('Distance to the CC (\mum)');
