...
M51 = zeros(4,Np+1);  M51(1,1)    = -sgm_Li*R_And*10;  M51(2,Np+1) = sgm_Li;  M51(3,Np+1) = Lx+sgm_Li*R_Ctd*10;
M52 = zeros(4,Np+1);  M52(1,1)    = 1;                 M52(3,Np+1) = 1;
M53 = zeros(4,Np+1);  M53(2,Np+1) = -sgm_e;            M53(3,Np+1) = Lx;
M54 = zeros(4,Np+1);  M54(3,Np+1) = 1;                 M54(4,1)    = 1;
...
% Entry 1:2*Np and 4*Np+4 are always zero; Entry 2*Np+1:3*Np and 4*Np+2 are constant;
% Entry 3*Np+1:4*Np are Li chemical potential in Ag particles; 
% Entry 4*Np+1 is the Li chemical potential in the anode; Entry 4*Np+3 is the Li chemical potential in the cathode;
YY  = [zeros(1,2*Np),FF*i_ave/10^4*ones(1,Np),zeros(1,Np),0,FF*i_ave/10^4,0,0]';
...
for it = 1:Nt-1
    for ip = 1 : nnp
        ...
        it_ionic(ip) = -m_cnt(idx(ip),it)*sgm_Li/FF;
        it_elec(ip)  = b_cnt(idx(ip),it)*sgm_e/FF;
    end
    figure(ifg)
