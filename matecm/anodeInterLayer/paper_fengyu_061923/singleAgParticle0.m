...
R_CC        = R_Mt/10.0;             % ASR of the charge-transfer resistance at the BL/CC interface, in unit Ohm*cm^2
...
yp_end      = 0.93/(1-0.93);         % The terminal value of y in AgLi_y based on the solubility limit
...
poreLi(idX) = maxPLi;
prs         = FF/Li_Vmol*depI(:,2:Nt)*R_Mt .* (1 - exp(-Li_K*Li_Vmol^2*pi*(dia(:,2:Nt)/2).^2/(R_Mt*FF^2*Hx^2*Lx*BL_por).*ttc(2:Nt)*10^7))/1000; % Pressure, in unit MPa
...
ifg = ifg + 1;
figure(ifg)
plot(ttp,-V0(2:Nt)/FF*10^3,'--k');
for ii = 1 : Np
    hold on
    plot(ttp,-eqmV(ii,2:Nt)/FF*10^3,'-b');
end
hold off
...
% hold on
% yyaxis left
plot(ttp,depSE(2:Nt)/i_ave,'-g',ttp,depCC(2:Nt)/i_ave,'-r');
for ii = 1 : Np
    hold on
    plot(ttp,depI(ii,2:Nt)/i_ave,'-b');
...
ifg = ifg + 1;
figure(ifg)
plot(ttp,LiSE(2:Nt),'--k',ttp,LiCC(2:Nt)+extrLi(2:Nt),'r');
xlabel('Charging time (Minutes)');
