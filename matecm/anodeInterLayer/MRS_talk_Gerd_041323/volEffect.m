...
for iN  = 8:8:256
    Ag_dia      = 0.02*ones(iN,1);       % The diameter of Ag particles, in unit um
    ...
    for it = 1:Nt-1
        ...
        if (abs(V0(it+1) - V0(it))<0.01) && abs(V0(it+1))/FF*10^3<10
            break;
        end
...
figure(ifg)
yyaxis left
plot(iniVol*100,stbV0,'--b');
xlabel('Vol% of Ag');
ylabel('stable IL voltage (mV)');
yyaxis right
plot(iniVol*100,stbTime,'--r');
ylabel('stable time (second)');
