...
% Generate Ag particles with the generator function:
% 1st: Cell information: [thickness, xy width]
% 2nd: Ag particle infromation: [volRatio, density, particleNumber, aveDia, sgmDia, minDia, maxDia]
% 3rd: 0 is for debugging, no real model will be generated; 1 is for generate model
Ag_dia = generateAg([Lx, Hx],[0.011, Ag_Mden, 1000, 0.04, 0.0022, 0.01, 0.1],0);         % The diameter of Ag particles, in unit um        
% Ag_dia = generateAg([Lx, Hx],[0.011, Ag_Mden, 1000, 0.04, 0.0001, 0.035, 0.045],0);         % The diameter of Ag particles, in unit um        
% Ag_dia = generateAg([Lx, Hx],[0.011, Ag_Mden, 1000, 0.02, 0.001, 0.01, 0.03],0);         % The diameter of Ag particles, in unit um   
...
ifg = ifg + 1;
figure(ifg)
plot(ttp,-V0(2:Nt)/FF*10^3, 'b', ttp,-eqmV(end,2:Nt)/FF*10^3,'k');
aa = [ttp',-V0(2:Nt)'/FF*10^3];
axis([0,1,0,60])
% for ii = 1 : Np
%     hold on
...
% xlabel('Charging time (Hours)');
% ylabel('y in AgLi_y');
% legend('smallAg','largeAg');
% 
% % Plot the volume ratio
% ifg = ifg + 1;
% figure(ifg)
% plot(ttp,rVol(1:Nt-1)*100);
% for ii = 1 : Np
%     hold on
%     plot(ttp,poreLip(ii,1:Nt-1)*100);
% end
% hold off
% xlabel('Charging time (Hours)');
% ylabel('volume ratio (%)');
% legend('AgLi','smallAg','largeAg');
% 
