...
% Ag_dia      = [0.03,0.05, 0.07]';                       % The diameter of particles, in unit um
Ag_dia      = [0.03, 0.05, 0.07, 0.09, 0.11, 0.13, 0.15]';      % The diameter of Ag particles, in unit um
...
i_exc       = 0.5;                   % According to Chiku's paper (Microelectrode studies on charger transfer), unit mA/cm^2
...
yp_end      = 0.93/(1-0.93);         % The terminal value of y in AgLi_y based on the solubility limit x=0.93
...
chemP_flag  = 2;                    % The chemical potential used, flag=1 means thermo-voltage, flag=2 means overshoot voltage
...
LiAg_freEng_chemPot_v2;
load('chemP_AgLi_db_v2.mat');
...
%     for ip = 1 : Np
%         if yc(ip,it) > 1
%             dia(ip,it)     = dia(ip,it-1); % If the diameter of AgLi_y is larger than the end state, diameter doesn't increase
%             poreLi(ip,it) = poreLi(ip,it-1) + pi/6*(Li_Vmol/Ag_Vmol)*Ag_dia(ip)^3*(yc(ip,it)-1);
%         else
%             dia(ip,it)   = Ag_dia(ip) * (Ag_Mden/AgLi_Mden(ip) * (1+Li_Mmol/Ag_Mmol*yc(ip,it)*yp_end)).^ (1/3); % The diameter of AgLi_y, in unit um
%         end
%     end
...
hold on
depSum = zeros(1,length(depI(1,2:Nt)));
for ii = 1 : Np
    depSum = depSum + pi*(Ag_dia(ii)/Hx)^2*depI(ii,2:Nt)/i_ave;
end
plot(ttp,depSum);
hold off
xlabel('Charging time (Hours)');
ylabel('Current density rate');
legend('SE/AgC','CC/AgC','Ag');
...
figure(ifg)
rVol   = sum(rVolp,1);
poreLi = sum(poreLip,1);
plot(ttp,rVol(1:Nt-1),ttp,poreLi(1:Nt-1));
xlabel('Charging time (Hours)');
ylabel('volume ratio');
legend('AgLi','poreLi');
...
figure(ifg)
xx   =linspace(0,2,100);
yy   = massDensity_AgLi(xx,yp_end,end_Mden,Ag_Mden,Ag_Mmol,Li_Mmol,Li_Mden);
Lixx =[0.33,1,1.5,1.6,2.25,3,yp_end];
royy =[7.94,5.86,4.72,4.44,3.64,3.1,end_Mden];
